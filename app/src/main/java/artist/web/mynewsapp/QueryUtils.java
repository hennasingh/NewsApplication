...
public class QueryUtils {
    ...
    private static final String API_SECTION = "sectionName";
    private static final String API_PUBLISHED_DATE = "webPublicationDate";
    ...
    /**
     * Query the URL and return a list of {@link NewsSegment} objects.
     */
    public static List<NewsSegment> fetchNewsItems(String requestUrl, Context context) {
        ...
        List<NewsSegment> newsSegment = extractFeatureFromJson(jsonResponse);
    ...
    private static URL createUrl(String stringUrl) {
        ...
        try {
            ...
            Log.e(LOG_TAG, "Problem with the URL String", e);
        }
        return url;
    }
    ...
    private static String makeHttpRequest(URL url) throws IOException {
        ...
        try {
            ...
            Log.e(LOG_TAG, "IOException retrieving JSON Results", e);
        } finally {
    ...
    /**
     * Return a list of {@link NewsSegment} objects retrieved from parsing a JSON response.
     */
    private static List<NewsSegment> extractFeatureFromJson(String newsJSON) {
        ...
        List<NewsSegment> newsItems = new ArrayList<>();
        ...
        try {
            ...
            jsonResults = baseJsonResponse.getJSONObject(API_RESPONSE);
            ...
            if (jsonResults.has(API_RESULTS)) {
                ...
                for (int i = 0; i < newsArray.length(); i++) {
                    ...
                    if (currentNewsItem.has(API_TITLE)) {
                        newsTitle = currentNewsItem.getString(API_TITLE);
                    }

                    if (currentNewsItem.has(API_SECTION)) {
                        newsSection = currentNewsItem.getString(API_SECTION);
                    }

                    if (currentNewsItem.has(API_PUBLISHED_DATE)) {
                        newsDate = currentNewsItem.getString(API_PUBLISHED_DATE);
                    }

                    if (currentNewsItem.has(API_WEBURL)) {
                        newsUrl = currentNewsItem.getString(API_WEBURL);
                    }

                    if (currentNewsItem.has(API_TAGS)) {
                        tagsArray = currentNewsItem.getJSONArray(API_TAGS);

                        if (tagsArray.length() > 0) {
                            for (int j = 0; j < 1; j++) {
                                newsTag = tagsArray.getJSONObject(j);
                                if (newsTag.has(API_TITLE)) {
                                    newsAuthor = newsTag.getString(API_TITLE);
                                }
                            }
                        }
                    }
                    ...
                    NewsSegment newsItem = new NewsSegment(
                            newsTitle,
                            newsSection,
                            newsDate,
                            newsAuthor,
